...
namespace SchoolBookPlatform.Controllers
{
    ...
    [Authorize]
    public class ChatController(ChatService chatService, ILogger<ChatController> logger) : Controller
    {
        [HttpGet]
        [Route("/Chat")]
        public IActionResult Index()
        {
            ...
            var threads = chatService.GetThreadsForUser(userId)?.ToList() ?? new List<ChatThread>();
        ...
        {
            var userId = User.Identity?.Name;
            var thread = chatService.GetThreadById(threadId, userId);
            
            if (thread == null)
            {
                logger.LogWarning($"Thread {threadId} not found for user {userId}");
                return NotFound();
            }
